...
const WorkItemForm = ({ workItemId, onSuccess }: WorkItemFormProps) => {
  ...
  const categories = ['Web Dev', 'Data Science / Analytics', 'Web3 Dev']
  ...
  useEffect(() => {
    const fetchWorkItem = async () => {
      ...
      try {
        setLoading(true);
        // Use service method instead of direct axios call
        const response = await workService.getWorkItem(workItemId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to save work item. Please try again.');
